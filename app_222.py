...
class Data_Base_Modelling():
    ...
    def Linear_Regression(self,x_train,y_train,x_test,y_test):
        ...
        for i,model in enumerate(pipelines):
            ...
            return f'RMSE score: {mean_squared_error(y_test,pred):.2f},\n R2 score: {r2_score(y_test,pred):.2f}'
    ...
    if choice == 'General EDA':
        ...
        if data is not None:
            ...
            if st.checkbox("Show dtypes"):
                st.write(dataframe.show_dtypes(df).astype(str))
...
# 				st.dataframe(new_df)
    ...
    elif choice == 'Auto Model Building':
        ...
        if data is not None:
            ...
            st.subheader("MODEL BUILDING")
            if st.checkbox("Build your BaseLine Model"):
                if set(y.unique()) == {0,1}:
                    st.write("Build these Classification Model")
                    ...
                    st.write("Build these Regression Model")
